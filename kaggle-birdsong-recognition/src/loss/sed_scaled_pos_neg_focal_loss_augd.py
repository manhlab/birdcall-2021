...
class SedScaledPosNegFocalLossAugd(nn.Module):
    ...
    def forward(self, y_pred, y_target):
        ...
        with torch.no_grad():
            ...
            y_primary_mask = torch.where(
                y_primary_true > 0.0, y_all_ones_mask, y_all_zeros_mask
            )
            y_primary_mask = torch.where(
                torch.rand(y_primary_mask.shape, device=y_primary_mask.device)
                <= self.primary_drop_out_factor,
                y_primary_mask,
                y_all_zeros_mask,
            )
