...
class NoisyControlledLoss(nn.Module):
    ...
    def forward(self, output, target, beta, primary_beta):
        ...
        ones_loss = (
            (((noisy_loss * target_all).sum() / (target_all.sum() + EPSILON_FP16)))
            * 4
            / 5
        )
        zeros_loss = (noisy_loss * (1 - target_all)).sum() / (
            (1 - target_all).sum() + EPSILON_FP16
        )  # * 5.0

        if target_primary is not None:
            primary_preds = output.view(-1)[
                [i * o + ix[0] for i, ix in enumerate(target_primary)]
            ]
            primary_loss = self.noisy_loss_fn(
        ...
        return loss, {
            ...
            "overall_target_loss": overall_target_loss,
        }
