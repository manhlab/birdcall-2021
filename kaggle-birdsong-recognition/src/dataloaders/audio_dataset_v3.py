                ...
                if np.random.random() > 0.2:
                    ...
                    for i in range(self.num_splits):
                        start_index = np.random.randint(
                            0, self.conf.sampling_rate // 10
                        )
                        indices = range(
                            start_index, duration - snip_duration, snip_duration
                        )
                        if len(indices) == 0:
                            indices = range(
                                0, duration - snip_duration + 1, snip_duration
                            )
                ...
                for j in range(len(y_snippets)):
                    ...
                    for i in range(num_divisions):
                        ...
                        for ci in np.random.choice(
                            selection_index, mixture_num, replace=False
                        ):
                            ci_y = y_snippets[j][ci : ci + division_length]
                            temp.append(ci_y)
                        temp = np.array(temp).mean(axis=0)
                        y_new[
                            i * division_length : i * division_length + division_length
                        ] = temp
                    y_snippets[j] = y_new
        else:
            self.isNoisy = False
