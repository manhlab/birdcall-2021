...
class AudioDatasetV2(Dataset):
    def __init__(
        ...
        if self.apply_mixer:
            ...
            self.label_values, self.label_probs = list(ebird_counts_dict.keys()), list(
                ebird_counts_dict.values()
            )
            self.dict_grp = {}
        ...
        self.data = list(
            df[["filename", "ebird_code", "secondary_labels", "xeno_source"]]
            .to_dict("index")
            .values()
        )
        ...
        if self.background_audio_dir is not None:
            for bk in background_audio_dir.glob("**/*.wav"):
                self.data.append({"filename": bk, "ebird_code": None})

        self.length = len(self.data)
        del df

        self.selection_index = list(
            range(0, 5 * self.conf.sampling_rate, self.conf.sampling_rate)
        )
            ...
            if duration - snip_duration <= 0:
                ...
                if np.random.random() > 0.2:
                    ...
                    for i in range(self.num_splits):
                        ...
                        indices = list(
                            range(start_index, duration - snip_duration, snip_duration)
                        )
                        if len(indices) == 0:
                            indices = list(
                                range(0, duration - snip_duration + 1, snip_duration)
                            )
                ...
                for j in range(len(y_snippets)):
                    ...
                    for i in range(5):
                        ...
                        for ci in np.random.choice(
                            self.selection_index, mixture_num, replace=False
                        ):
                            ci_y = y_snippets[j][ci : ci + self.conf.sampling_rate]
                            temp.append(ci_y)
                        temp = np.array(temp).mean(axis=0)
                        y_new[
                            i * self.conf.sampling_rate : i * self.conf.sampling_rate
