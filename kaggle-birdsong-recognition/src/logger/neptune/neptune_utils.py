...
def training_iteration(writer):
    def print_training_iteration(engine):
        ...
        writer.experiment.log_metric(
            "train/loss", iteration, engine.state.output["loss"]
        )
        ...
        if slack_url is not None:
            try:
                requests.post(
                    slack_url,
                    json={
                        "text": f"[TRAIN] {engine.state.epoch}: {metrics['train_avg_loss']}"
                    },
                )
            except:
                pass
        ...
        if slack_url is not None:
            try:
                requests.post(
                    slack_url,
                    json={
                        "text": f"[VALID] {trainer.state.epoch}: {metrics['valid_avg_loss']}"
                    },
                )
            except:
                pass
