...
class MainEngineV2(BaseEngine):
    ...
    def _init_criterion_function(self):
        if self.hparams.criterion_name == "bce":
            ...
            self.criterion = NoisyCuratedLoss(
                noisy_type=self.hparams.criterion_name,
            ...
            self.criterion = NoisyCuratedLossTargetted(
                noisy_type=self.hparams.criterion_name,
                beta=self.hparams.beta,
                q=self.hparams.q,
            )
    ...
    def _init_metrics(self):
        ...
        if "f1score50" in self.hparams.metrics:
            ...
            self.validation_metrics["f1score50"] = CustomF1Score(
                threshold=0.5, output_transform=lambda x: (x["y_pred"], x["y"])
            )

        if "f1score60" in self.hparams.metrics:
            from metrics.custom_f1score import CustomF1Score

            self.validation_metrics["f1score60"] = CustomF1Score(
                threshold=0.6, output_transform=lambda x: (x["y_pred"], x["y"])
            )

        if "f1score70" in self.hparams.metrics:
            from metrics.custom_f1score import CustomF1Score

            self.validation_metrics["f1score70"] = CustomF1Score(
                threshold=0.7, output_transform=lambda x: (x["y_pred"], x["y"])
            )

        if "lraps" in self.hparams.metrics:
            from metrics.custom_lraps import CustomLRAPS

            self.validation_metrics["lraps"] = CustomLRAPS(
        ...
        if "test" in self.hparams.metrics:
            ...
            self.test_metrics["test"] = CustomTest(
                ...
                output_transform=lambda x: (x["y_pred"], x["input"]),
            )
