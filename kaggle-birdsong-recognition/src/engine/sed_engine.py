...
class SedEngine(BaseEngine):
    ...
    def prepare_batch(self, batch, mode="valid"):
        if mode == "train":
            x = convert_tensor(
                batch["waveforms"], device=self.device, non_blocking=True
            )
            bs, c, s = x.shape

            all_labels = convert_tensor(
                batch["all_labels"], device=self.device, non_blocking=True
            )
            primary_labels = convert_tensor(
                batch["primary_labels"], device=self.device, non_blocking=True
            )
            secondary_labels = convert_tensor(
                batch["secondary_labels"], device=self.device, non_blocking=True
            )

            if self.mixup_augmenter is not None:
                mixup_lambda = self.mixup_augmenter.get_lambda(
                    batch_size=bs * c, device=x.device
                )
                bs_al, s_al, c_al = all_labels.shape
                all_labels = do_mixup(
                    all_labels.reshape(bs_al * s_al, c_al), mixup_lambda
                ).reshape((bs_al * s_al) // 2, 1, c_al)
                primary_labels = do_mixup(
                    primary_labels.reshape(bs_al * s_al, c_al), mixup_lambda
                ).reshape((bs_al * s_al) // 2, 1, c_al)
                secondary_labels = do_mixup(
                    secondary_labels.reshape(bs_al * s_al, c_al), mixup_lambda
                ).reshape((bs_al * s_al) // 2, 1, c_al)
            else:
                mixup_lambda = None
            ...
            return (
                (convert_tensor(x, device=self.device, non_blocking=True), None),
                {
                    "all_labels": convert_tensor(
                        batch["all_labels"], device=self.device, non_blocking=True
                    ),
    ...
    def _init_criterion_function(self):
        if self.hparams.criterion_name == "focal_loss_standard":
            ...
            from loss.sed_scaled_pos_neg_focal_loss_augd import (
                SedScaledPosNegFocalLossAugd,
            )

            self.criterion = SedScaledPosNegFocalLossAugd(
    ...
    def _init_scheduler(self):
        if self.hparams.scheduler_name == "none":
            ...
            scheduler_1 = LinearCyclicalScheduler(
                self.optimizer,
                "lr",
                start_value=lr * self.hparams.lr_scale_factor,
                end_value=lr,
                cycle_size=epoch_length * 2,
            )
            scheduler_2 = CosineAnnealingScheduler(
                self.optimizer,
                "lr",
                start_value=lr,
                end_value=lr * self.hparams.lr_scale_factor,
                cycle_size=num_epochs * epoch_length,
            )
            durations = [
                epoch_length,
            ]
            self.scheduler = ConcatScheduler(
    ...
    def _init_metrics(self):
        ...
        def add_criterion_metrics(loss_key):
            self.train_metrics[f"train_{loss_key}"] = RunningAverage(
                output_transform=lambda x: x["dict_loss"][f"{loss_key}"]
            )
            self.validation_metrics[f"valid_{loss_key}"] = RunningAverage(
                output_transform=lambda x: x["dict_loss"][f"{loss_key}"]
            )
        ...
        if "f1score_clip" in self.hparams.metrics:
            ...
            self.validation_metrics[f"f1score_clip"] = SedF1ScoreClip(
                threshold=0.5,
                output_transform=lambda x: (x["y_pred"], x["y"]["all_labels"]),
            )
        ...
        if "f1score_frame" in self.hparams.metrics:
            ...
            self.validation_metrics[f"f1score_frame"] = SedF1ScoreFrame(
                threshold=0.5,
                output_transform=lambda x: (x["y_pred"], x["y"]["all_labels"]),
            )
        ...
        if "test" in self.hparams.metrics:
            ...
            self.test_metrics["test"] = SedTest(
    ...
    def _init_model(self):
        if self.hparams.model_name == "sed_ccnn14att":
            ...
            if self.hparams.pretrained_path is not None:
                weights = torch.load(self.hparams.pretrained_path, map_location="cpu")[
                    "model"
                ]
    ...
    def _init_augmentation(self):
        if self.hparams.aug_name == "default":
            ...
            self.tfms = get_transforms(
                bckgrd_aug_dir=self.hparams.bckgrd_aug_dir,
                secondary_bckgrd_aug_dir=self.hparams.secondary_bckgrd_aug_dir,
            )

        if self.hparams.apply_mixup:
            from augmentations.mixup import Mixup, do_mixup

            self.mixup_augmenter = Mixup(mixup_alpha=1.0)
        else:
    ...
    def _init_train_datalader(self):
        ...
        self.train_ds = SedDataset(
            **self.hparams.train_ds_params, transform=self.tfms["train"]
        )

    def _init_valid_dataloader(self):
        from dataloaders.sed_dataset import SedDataset

        self.valid_ds = SedDataset(
            **self.hparams.valid_ds_params, transform=self.tfms["valid"]
        )

    def _init_test_dataloader(self):
        from dataloaders.sed_dataset import SedDataset

        self.test_ds = SedDataset(
            **self.hparams.test_ds_params, transform=self.tfms["valid"]
        )
