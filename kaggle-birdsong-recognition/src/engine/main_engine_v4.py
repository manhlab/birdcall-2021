...
class MainEngineV4(BaseEngine):
    ...
    def prepare_batch(self, batch, mode="valid"):
        if mode == "train":
            ...
            return (
                ...
                (convert_tensor(y[0], device=self.device, non_blocking=True), y[1]),
            )
        elif mode == "valid":
            x, y = batch["images"], (batch["coded_labels"], batch["primary_codes"])
            return (
            ...
            return (convert_tensor(x, device=self.device, non_blocking=True), (inputs))
    ...
    def _init_optimizer(self):
        if self.hparams.optimizer_name == "adamw":
            ...
            self.optimizer = torch.optim.AdamW(
                self.model.parameters(), lr=self.hparams.lr, weight_decay=0.0
            )
    ...
    def _init_criterion_function(self):
        if self.hparams.criterion_name == "noisy_controlled_loss":
            ...
            self.criterion = BCEControlledLoss(
                primary_loss_factor=self.hparams.primary_loss_factor
            )
            ...
            self.criterion = FocalControlledStandardLoss(
                gamma=self.hparams.gamma, alpha=self.hparams.alpha
            )
        elif self.hparams.criterion_name == "focal_mse_loss":
            from loss.focal_mse_loss import FocalMSELoss

            self.criterion = FocalMSELoss(
                gamma=self.hparams.gamma, alpha=self.hparams.alpha
            )
    ...
    def _init_metrics(self):
        ...
        def add_criterion_metrics(loss_key, i):
            self.train_metrics[f"train_{loss_key}_{i}"] = RunningAverage(
                output_transform=lambda x: x["dict_loss"][f"{loss_key}_{i}"]
            )
            self.validation_metrics[f"valid_{loss_key}_{i}"] = RunningAverage(
                output_transform=lambda x: x["dict_loss"][f"{loss_key}_{i}"]
            )
        ...
        if "f1score50" in self.hparams.metrics:
            ...
            for i in range(1):
                self.validation_metrics[f"f1score50_{i}"] = CustomF1Score(
                    threshold=0.5, output_transform=lambda x: (x["y_pred"][i], x["y"])
                )
        ...
        if "f1score60" in self.hparams.metrics:
            ...
            for i in range(1):
                self.validation_metrics[f"f1score60_{i}"] = CustomF1Score(
                    threshold=0.6, output_transform=lambda x: (x["y_pred"][i], x["y"])
                )
        ...
        if "f1score70" in self.hparams.metrics:
            ...
            for i in range(1):
                self.validation_metrics[f"f1score70_{i}"] = CustomF1Score(
                    threshold=0.7, output_transform=lambda x: (x["y_pred"][i], x["y"])
                )

        if "test" in self.hparams.metrics:
            from metrics.custom_test import CustomTest

            self.test_metrics["test"] = CustomTest(
                save_dir=self.hparams.save_dir,
                combine_files=self.hparams.combine_files,
                output_transform=lambda x: (x["y_pred"][0], x["input"]),
            )
