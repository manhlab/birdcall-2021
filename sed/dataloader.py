    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            if random.random() < 0.9:
                images = images + (
                    np.random.sample((self.n_mels, self.len_chack)).astype(np.float32)
                    + 9
            ...
            if random.random() < 0.9:
                ...
                images[a:b, :] = images[a:b, :] + (
                    np.random.sample((b - a, self.len_chack)).astype(np.float32) + 9
        ...
        if background_audio:
            ...
            mel = np.concatenate((mel, np.zeros((self.n_mels, self.len_chack))), axis=1)
...
class BirdClefDataset_V2(Dataset):
    def __init__(
        ...
        self.len_chack = 626
        self.stop_border = (
            0.8  # Probability of stopping mixing | Вероятность прервать смешивание
        )
        self.level_noise = 0.05  # level noise | Уровень шума
        self.div_coef = (
            100  # signal amplification during mixing | Усиления сигнала при смешивании
    ...
    def __getitem__(self, idx):
        ...
        for i, idy in enumerate([idx, idx2, idx3]):
            ...
            t_pobs = melspecs.item().get("probs")
            try:
                melspecs = melspecs.item().get("images")[
                    np.random.choice(len(t_pobs), size=1, p=t_pobs)[0]
                ]
            except:
