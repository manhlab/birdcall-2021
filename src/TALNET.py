...
class ConvBlockTALNet(nn.Conv2d):
    ...
    def __init__(
        ...
        if norm == "GN":
            ...
            self.norm_layer = nn.GroupNorm(
                num_channels=self.out_channels, num_groups=32
            )
        elif norm == "BN":
            self.norm = True
    ...
    def forward(self, x):
        ...
        weight = self.weight - self.weight.view(self.weight.size(0), -1, 1, 1).mean(
            1, keepdim=True
        )
        std = (
            weight.view(weight.size(0), -1, 1, 1).std(dim=1, keepdim=True) + 1e-5
        )  # Avoid 0 div
...
class TALNetV2(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention_1 = MultiHead(
        ...
        self.pooling_head = Pooling_Head(
            self.embedding_size, self.output_size, self.pooling
        )
...
class TALNetV2_meta(nn.Module):
    ...
    def __init__(
        ...
        self.self_attention_meta = MultiHead(
        ...
        self.self_attention_meta_talnet = MultiHead(
            ...
            self.meta_emb,
            self.meta_emb,
            self.dropout_transfo,
        )
        self.fc_prob = Normed_Linear(
            self.embedding_size + self.meta_emb * self.num_meta, self.output_size
        )
        self.pooling_head = Pooling_Head(
            self.embedding_size + self.meta_emb * self.num_meta,
            self.output_size,
            self.pooling,
        )
...
class TALNetV3(nn.Module):
    def __init__(
        ...
        self.fc_prob = nn.Linear(
            self.embedding_size * 2 + self.meta_emb * self.num_meta, self.output_size
        )
        if self.pooling == "att":
